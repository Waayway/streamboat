...
| `mopidy-tidal` | https://github.com/EbbLabs/mopidy-tidal (formerly `tehkillerbee/mopidy-tidal`) | `18abb3b` | Headless Mopidy backend. Localhost HTTP relay cache proxy with SQLite chunked insertion and full `Range`/`Content-Range` support — the model for resuming a direct-URL stream after a mid-track failure. |
...
| `python-tidal` | https://github.com/EbbLabs/python-tidal (formerly `tamland/python-tidal`) | `9c41fbe` | The de-facto reference unofficial-API client. `Quality`/`AudioMode`/`MediaMetadataTags`/`ManifestMimeType` enums, `DashInfo` MPD field extraction (does *not* read `Representation@id`), MPD→HLS synthesis, `$Number$` segment numbering starting at 0 — with two further independent bugs beyond the start-index one: it under-counts `SegmentTimeline/S@r` by one segment per run (adds `r`, not `r+1`), and its `get_urls()` likely never emits the init segment at all while over-generating one trailing media URL (`tidal-manifest-api.md` §4). No `HI_RES` value remains in its `Quality` enum (removed, MQA-related). |
...
| `tidal-connect` | https://github.com/GioF71/tidal-connect | `05ea5d7` | Docker wrapper around a closed-source TIDAL Connect binary. Raspberry Pi HDMI/I2S-HAT ALSA configs, card-name-not-index device selection, ALSA softvol mixer-naming collision warning. **Correction, previously overstated here as refuting the effective-ceiling claim**: it historically supported up to 24/48 hi-res plus MQA (in-app unfolding to 24/88-24/96) — that is the *capability* figure — but the same README paragraph says content above 16/44 is "currently inexistent on Tidal," so it is **effectively LOSSLESS-only [16-bit/44.1 kHz] since MQA's removal** in practice. Say both together, per `tidal-client-features/references/remote-playback-connect-controls.md` §2, the canonical formulation. |
...
**Correction: `linuxiac.com` is blocked from this research environment, not fetched.** An earlier
pass in this skill presented the `linuxiac.com` rows below as directly fetched web sources; a
re-check found the domain returns `EGRESS_BLOCKED`, matching `tech-stack-evaluation`'s independent
finding (`tech-stack-evaluation/references/verification-notes.md` §3,
`tech-stack-evaluation/references/sources.md`) that `linuxiac.com` is among the domains blocked or
failed in this environment. Downgrade every `linuxiac.com`-only row below one confidence tier —
treat as search-index-summary confidence, not direct-fetch confidence — and prefer the paired
`9to5linux.com`/`linuxcompatible.org`/`phoronix.com`/`discourse.gstreamer.org` citation in the same
row where one exists. This does not affect the two engineering floors this skill actually gates
code on: `>= 1.26.10` for FLAC-in-DASH and `>= 1.28` for `wasapi2sink exclusive` both have
independent primary sources elsewhere in this table (Debian's package archive; the plugin's own
gtk-doc `Since:` tag, read directly, not via news coverage).
...
| `https://www.phoronix.com/news/GStreamer-1.28`, `https://lists.freedesktop.org/archives/gstreamer-devel/2026-January/082207.html` | GStreamer 1.28.0 (27 Jan 2026) — wasapi2 IMMDevice port. The "dashdemux2 seeking-with-gaps fix" this row was also cited for is **downgraded to unverified**: neither URL nor `gstreamer.freedesktop.org` is reachable from this environment, and no other source corroborates it (`playback-behavior.md` §8) |
| `https://discourse.gstreamer.org/t/gstreamer-1-28-0-new-major-feature-release/5700` | Re-confirms 1.28.0's date and the WASAPI2 IMMDevice port from an unblocked source; does not mention dashdemux2 gap-seeking either |
...
| `https://linuxiac.com/gstreamer-1-28-5-released-with-security-and-playback-fixes/`, `https://linuxiac.com/gstreamer-1-28-6-adds-h-266-mp4-muxing-and-ffmpeg-9-0-support/`, `https://9to5linux.com/gstreamer-1-28-6-adds-h-266-muxing-support-to-the-rust-mp4-muxers`, `linuxcompatible.org` ("GStreamer 1.28.6 Drops: Final 1.28 Release") | **Corrects a prior "1.28.2/1.28.3 is current" statement in this skill.** The 1.28 series shipped 1.28.5 and then 1.28.6 (5 Aug 2026) — `linuxcompatible.org` called 1.28.6 "the final 1.28 release," which the next row shows was wrong; 1.28.6 adds FFmpeg 9.0 support, matching the `ffmpeg-next` 9.0.0 crate recommendation |
| `https://linuxiac.com/gstreamer-1-28-7-released-with-security-and-playback-fixes/`, `https://9to5linux.com/gstreamer-1-28-7-open-source-multimedia-framework-adds-support-for-opencv-5` (both dated 2026-09-08) | **Supersedes the row above's "1.28.6 is final" claim** — 1.28.7 shipped after it. Do not treat any specific 1.28.x number in this skill as settled; re-check the current point release. The two engineering floors this skill actually gates code on (>= 1.26.10 FLAC-in-DASH, >= 1.28 `wasapi2sink exclusive`) are unaffected by which 1.28.x point release is current |
...
| ISO/IEC 23009-1 (MPEG-DASH) §5.3.9.4.4 "Template-based Segment URL construction", §5.3.9.6 "Segment timeline" | The full `SegmentTemplate` identifier set (`$Number$`, `$Time$`, `$Bandwidth$`, `$RepresentationID$`, `$$`, printf width tags) and the `SegmentTimeline` timing model — no reference client implements more than a `$Number$` subset (`tidal-manifest-api.md` §4) |
| `docs/research/tech-stack.md:268,1383` (this repository) | Recommends GStreamer as primary engine with libmpv as backend #2 (`:1383`, the `Audio engine` row: `\| gstreamer 0.25 + gstreamer-app, behind an AudioEngine trait; libmpv as backend #2 \|`), and `--gapless-audio=no` for "strict bit-perfect" (`:268`, the libmpv row) — directly contradicts this skill's/report's engine and gapless-flag recommendation; flagged as an unresolved owner decision, `stacks-comparison.md` §3. (A prior version of this citation read `:225,1206-1207,1390` — all three numbers were wrong; `:1390` pointed at the unrelated `Async \| tokio` row.) |
| `docs/research/headless-connect.md:1726-1727,2207` (this repository) | Windows services run in session 0 with no interactive audio endpoint; macOS needs a per-user LaunchAgent, not a LaunchDaemon — both marked unverified there; restated in `os-integration.md` §4 for its audio-layer consequence |

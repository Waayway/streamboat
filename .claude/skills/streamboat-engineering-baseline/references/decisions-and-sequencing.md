...
baseline table, the 24-item do-now checklist, the packaging sequence, and the stack-blocked table).

## Contents

1. Baseline: what the reference projects actually do (comparison table)
2. Decide and do now (stack-independent) — 24 items
...
8. **Apple Developer Program ($99/year) and Windows signing (reported ~$120/year Trusted Signing
   ...
   caveat).** Who pays, and are macOS/Windows first-class targets from v1 or best-effort?
...
- **Azure Trusted Signing / Artifact Signing pricing and eligibility** ($9.99/$99.99 per month,
  ...
  `learn.microsoft.com` are blocked from this environment. Re-verify before it drives a cost
  decision (`packaging-and-distribution.md` §5, §6).
...
- **The Flathub linter's specific "never granted" exception rule for
  ...
  source (`packaging-and-distribution.md` §1); only the specific linter-rule wording about the
  talk-name exception is unverified.

---
name: tidal-api
description: Wire-level knowledge of the TIDAL APIs streamboat talks to — the unofficial api.tidal.com v1/v2 surface every OSS client (High Tide, Sone, Strawberry, python-tidal) actually streams through, plus the official openapi.tidal.com/v2 Developer API — covering OAuth device-code and PKCE login, client-ID provenance and rotation risk, token refresh/storage/logout, request headers and rate limiting, the playbackinfopostpaywall call and manifest/encryption/quality-cascade rules, catalog and pages endpoints, playlist/favorites CRUD, lyrics, images, play-reporting to ec.tidal.com, and the legal/ToS posture around all of it. Use this whenever writing or reviewing code that calls api.tidal.com, openapi.tidal.com, auth.tidal.com, or login.tidal.com; whenever touching OAuth/login/token-refresh/keyring code, a playback-info or manifest parser, quality-tier or bitrate logic, encryption/DRM handling, playlist or favorites mutation, search, the Home/Explore page renderer, lyrics, cover-art URLs, rate-limit or retry logic, or play-history/scrobbling; and whenever the task mentions TIDAL, python-tidal, tidalapi, High Tide, Sone, Strawberry, playbackinfopostpaywall, urlpostpaywall, manifest, audioquality, HI_RES_LOSSLESS, client_id, device code, PKCE, subStatus, or countryCode. Do not rely on general music-streaming-API knowledge here — TIDAL's actual wire format (camelCase device-flow responses, literal `+` in PKCE scope strings, silent quality downgrades, DRM-gated official manifests) differs from generic REST/OAuth assumptions in ways that cause silent, hard-to-attribute bugs.
---
...
- **TIDAL Connect is permanently out of scope** — no open protocol exists to build against, in
  either direction (`references/legal-and-landscape.md` §6). Not a "later" feature; a closed door.

## The facts that cause silent bugs if you get them wrong

| # | Trap | The fix |
| --- | --- | --- |
| 1 | Over-requesting `audioquality` returns **HTTP 200 with a silently downgraded `audioQuality`**, never an error. | Always display the response's `audioQuality`, never the one you requested. The quality cascade exists to handle *errors*, not downgrades. `references/playback.md` §1, §4. |
...
| 3 | `subStatus 4034` (`NO_CONTENT_MATCHING_CLIENT`) is **client/tier-scoped, not permanently terminal** — Sone's own blanket "terminal" set is wrong here and must not be copied as-is. | Retry once at a lower tier or a different client id before giving up; never permanently blacklist a track on `4034`. `references/transport.md` §6. |

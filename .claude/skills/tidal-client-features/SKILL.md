...
`references/browse-pages-screens.md` §4).
...
| 12 | `souvlaki` 0.8.3 actually covers all three OSes in one crate — Linux MPRIS (`dbus`+`dbus-crossroads`), Windows SMTC (`windows` crate), macOS `MPNowPlayingInfoCenter` (`cocoa`/`objc`/`dispatch`) are all resolved deps of a single `souvlaki` install (`ref:sone-windows/src-tauri/Cargo.lock:5041-5055`). The reference clients still split by platform anyway: sone-windows (a third-party fork of Sone, not Sone's own Windows port — `references/sources.md`) uses it for Windows only; Sone/Linux uses `mpris-server` instead; High Tide hand-rolls Python D-Bus. | Don't budget three integrations by default — evaluate `souvlaki` per platform first (a dedicated crate like `mpris-server` may still win on Linux fidelity), and pick deliberately. `references/remote-playback-connect-controls.md` §4. |
...
| 14 | The action count is **694 entries across 48 namespaces (679 namespaced + 15 bare action names, e.g. `noop`, `ADD_ARTISTS_TO_CATEGORY`)**, not "~600 across ~40" — and that count is dated to one build (see version caveat above). Reproduce it as `grep -oE '"[^"]+"' actionTypes.ts \| wc -l` scoped to the `actionTypes` array only — grepping the whole file over-counts by one (`"@inrixia/helpers"` in the import line). | Cite the number and the build together whenever you quote it. |

...
| Feature | Tier | API availability / who implements | Rationale |
| --- | --- | --- | --- |
| Be controlled by MPRIS (Linux) / SMTC (Windows) / Now Playing (macOS) | **MVP** | **No single crate unifies all three.** sone-windows: `souvlaki` for Windows SMTC only (ref:sone-windows/src-tauri/Cargo.toml:64); Sone/Linux: `mpris-server` (ref:sone/src-tauri/Cargo.toml:66); High Tide: Python D-Bus MPRIS (ref:high-tide/src/mpris.py); tidal-hifi: own MPRIS service | Expected baseline OS integration on every desktop platform; budget three integrations, not one |
...
| Ads / ad-supported playback | out-of-scope | — **[uncertain]** | No live ad-supported tier exists to build against |

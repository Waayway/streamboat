...
storage (D-026), and the exclusive-mode ALSA writer (D-017, D-018) are built: see
`docs/architecture.md` for the crate contents, the implementation choices fixed along the way (seek
flags, DASH feeding, `dashdemux2` demotion, token file format, master-key resolution order, PKCE
capture mechanisms, protocol version, the ALSA writer's format/rate/reopen rules) and the
not-yet-built list in decision order. The libmpv backend, the control API and the iced shell are
next.

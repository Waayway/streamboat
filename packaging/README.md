...
D-016 puts GStreamer on Linux and libmpv on Windows and macOS behind one
`Engine` trait; `streamboat_player::default_engine` picks the compiled-in
backend, and each binary forwards the choice through its own Cargo features
(`gstreamer`, the default, and `mpv`). Linux builds use the defaults; the
Windows and macOS release jobs build with
`--no-default-features --features streamboat-desktop/mpv,streamboat-server/mpv`
(see `.github/workflows/release.yml`), which links libmpv plus the SMTC and
NowPlaying media-control adapters and nothing from GStreamer. That
invocation is verified on Linux (libmpv is installable there too); the
Windows and macOS jobs themselves have not run yet — no runner for either
existed while this directory was written.

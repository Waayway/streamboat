...
use streamboat_player::{GstEngine, Player, PlayerConfig, PlayerHandle};
...
fn spawn_login(ctx: &Context, handle: &PlayerHandle) {
        ...
        match start_device_flow(&api).await {
            Ok(auth) => {
                ...
                match wait_for_device_token(&api, &auth, || true).await {
                    Ok(t) => login_handle.publish(Event::AuthOk {
                        user_id: t.user_id,
